...
export function handleStatFilters(){
    ...
    filtreInputRange.addEventListener('input', (event)=>{
        ...
        const filtroGenre = document.querySelector('.filtro__genre-btn--ativo').innerText;
        
        const currentInputStatSpan = currentInputStat.querySelector('.price-range-value');
        const  currentInputDataValue = event.target.valueAsNumber;
        currentInputStatSpan.innerText = currentInputDataValue;

        const arrayAlbunsFiltred = albumList.filter(({price,genre})=>{
            if(filtroGenre == 'Todos'){
                return price <= currentInputDataValue;
            }
            return (price <= currentInputDataValue && 
                    genre == filtroGenre || genre === 'Todos' && price  <= currentInputDataValue );
            });
